    ...
    <nav className={`navbar ${scrolled ? 'navbar-scrolled' : ''}`}>
      <div className="navbar-inner">
        ...
        <div className="nav-right">
          {isAdmin && (
            <button className="nav-bell-btn" onClick={() => onNav('admin')} title="Admin Dashboard" style={{ position: 'relative', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', marginRight: '6px' }}>
              <Bell size={20} />
              {pendingCount > 0 && <span style={{ position: 'absolute', top: '-2px', right: '-2px', width: '9px', height: '9px', backgroundColor: '#ef4444', border: '2px solid #fff', borderRadius: '50%' }} />}
            </button>
          )}
          {azureUser && (
...
/* ─────────────────────────────────────
   MARKETPLACE
───────────────────────────────────── */
/* Every real category gets its own chip. Data Engineering, Design and Reporting
   used to be collapsed into a single "Tech" chip, which meant a chat answer
   ("the 3 Data Engineering apps are…") disagreed with the 6 cards the grid then
   showed. One chip per category keeps the assistant and the grid consistent. */
const ALL_CATEGORIES = ['All', ...Array.from(
  new Set(apps.map((a) => a.category)),
).sort()];

/* Hero stats derived from the catalogue so they can never drift from `apps`.
   The app count is rounded down to the nearest five and suffixed with "+" —
   "25+" reads as a claim rather than an exact figure that shifts every time an
   application is added. */
const TOTAL_APPS_LABEL = `${Math.floor(apps.length / 5) * 5}+`;
const TOTAL_CATEGORIES = new Set(apps.map((a) => a.category)).size;

type SortKey = 'featured' | 'name-asc' | 'name-desc' | 'category';
...
/** Ignore the ™ suffix so "aiDE™" sorts next to "AeroIntel", not after it. */
const sortVal = (s: string) => s.replace(/[™®]/g, '').trim().toLowerCase();
...
const Marketplace = ({
  ...
  onAppClick: (a: AppDetail) => void;
  restoreScrollY?: number;
  /** Owned by App so the assistant can drive the filter. */
  cat: string;
  onCatChange: (c: string) => void;
  /** Explicit app ids to show — set when the assistant names several apps. */
  ...
  const filtered = apps.filter((a) => {
    const matchApp = !appFilter?.length || appFilter.includes(a.id);
    ...
    return matchApp && matchCat && matchQ;
  });

  // `filtered` is already a fresh array, so sorting in place never touches `apps`.
  // 'featured' keeps the curated order the apps are declared in.
  if (sort === 'name-asc') {
    filtered.sort((a, b) => sortVal(a.name).localeCompare(sortVal(b.name)));
  } else if (sort === 'name-desc') {
    filtered.sort((a, b) => sortVal(b.name).localeCompare(sortVal(a.name)));
  } else if (sort === 'category') {
    filtered.sort((a, b) =>
      sortVal(a.category).localeCompare(sortVal(b.category)) ||
      sortVal(a.name).localeCompare(sortVal(b.name))
    );
  }
    ...
    <section className="marketplace-page">
      <div className="mp-hero">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="mp-sub">Enterprise-grade AI applications across various industries</p>
          <div className="mp-stats-row">
            {[
              { val: TOTAL_APPS_LABEL, label: 'Total Apps' },
              { val: `${TOTAL_CATEGORIES}`, label: 'Categories' },
      ...
      <div className="mp-controls">
        <div className="mp-controls-row">
          ...
          <div className="mp-sort-wrap">
            <ArrowUpDown size={15} className="mp-sort-icon" />
            <select
              className="mp-sort"
              value={sort}
            ...
            <ChevronDown size={15} className="mp-sort-caret" />
          </div>
        </div>
        <div className="mp-cats" id="mp-categories">
        ...
        {/* Shown when the assistant narrowed the grid to specific applications,
            so a short grid never looks like a bug. */}
        {!!appFilter?.length && (
          <motion.div
            className="mp-assist-banner"
          ...
          >
            ...
            <span>
              ...
              {filtered.length === 1 ? 'application' : 'applications'} from your assistant question
            </span>
            <button onClick={onClearAppFilter}>
              <X size={12} /> Clear
            </button>
          </motion.div>
        )}
      ...
      <div className="mp-grid">
        ...
        {filtered.length === 0 && (
          <div className="mp-empty">
            ...
            <button className="btn-outline" onClick={() => { setQuery(''); onCatChange('All'); }}>Clear filters</button>
          </div>
        )}
      </div>
    </section>
...
const AppDetails = ({
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      ...
      // Without this check a failed request still showed "Request Sent!", so the
      // user believed access was requested when nothing had been recorded.
...
export default function App() {
  ...
  /**
   * Driven by the assistant: a category question filters the grid to match the
   * answer. Returns to the marketplace first if the user is on another page,
   * then scrolls the category row into view so the change is actually visible
   * (the hero is tall enough to hide it otherwise).
   */
  const revealMarketplace = () => {
    setPage('marketplace');
    savedScrollY.current = 0;
    requestAnimationFrame(() => {
      document
        .getElementById('mp-categories')
        ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  };

  const filterByCategory = (cat: string) => {
    setAppFilter(null); // a category and an explicit app list are exclusive
    setCategory(cat);
    revealMarketplace();
  };

  /** The assistant named several applications — show exactly those. */
  const showApps = (ids: string[]) => {
    setCategory('All');
    setAppFilter(ids);
    revealMarketplace();
  };

  const clearAppFilter = () => setAppFilter(null);

  /** Picking a category chip by hand should drop an assistant-driven app list. */
  ...
  return (
    <>
      ...
      {/* The assistant drives navigation: application names route to a detail
          page, and category questions filter the marketplace grid. */}
      <Chatbot
        onOpenApp={openApp}
