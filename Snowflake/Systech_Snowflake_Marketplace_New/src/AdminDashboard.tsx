                            ...
                            {filtered.length === 0 ? (
                                ...
                                filtered.map(opt => (
                                    <label key={opt} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', cursor: 'pointer', fontSize: '13px', color: '#334155', borderRadius: '4px' }}>
                                        <input type="checkbox" checked={selected.includes(opt)} onChange={() => toggle(opt)} style={{ width: '14px', height: '14px', accentColor: '#249edc' }} />
...
export default function AdminDashboard({ onBack }: { onBack: () => void }) {
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
            ...
            {/* ── Snowflake Hero Banner ── */}
            <div style={{
                background: 'linear-gradient(160deg, #001824 0%, #003545 60%, #11567F 100%)',
                paddingTop: '100px', paddingBottom: '48px',
                paddingLeft: '40px', paddingRight: '40px',
                position: 'relative', overflow: 'hidden',
            }}>
                {/* dot-grid watermark */}
                <div style={{ position: 'absolute', right: 0, top: 0, width: '420px', height: '220px', opacity: 0.06, backgroundImage: 'radial-gradient(circle, #6ec9eb 1px, transparent 1px)', backgroundSize: '22px 22px', pointerEvents: 'none' }} />

                <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: '7px', background: 'none', border: 'none', color: 'rgba(255,255,255,0.6)', cursor: 'pointer', marginBottom: '28px', fontSize: '14px', fontWeight: 500, padding: 0 }}>
                    <ChevronLeft size={16} /> Back to Marketplace
                </button>

                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <Shield size={32} color="#6ec9eb" />
                    <div>
                        <h1 style={{ fontSize: '28px', fontWeight: 700, margin: '0 0 6px 0', color: '#fff', letterSpacing: '-0.5px' }}>Access Requests</h1>
                        <p style={{ margin: 0, color: 'rgba(255,255,255,0.60)', fontSize: '14px' }}>Manage user application permissions efficiently.</p>
                    </div>
                </div>
            </div>

            {/* ── Content ── */}
            <div className="admin-page" style={{ paddingTop: '32px', paddingBottom: '60px', paddingLeft: '40px', paddingRight: '40px', maxWidth: '1240px', margin: '0 auto', color: '#1e293b' }}>
                ...
                {['all', 'pending', 'approved', 'revoked'].map(tab => (
                    <button
                        ...
                        style={{
                            ...
                            background: 'none',
                            border: 'none',
                            borderBottom: statusTab === tab ? '2px solid #249edc' : '2px solid transparent',
                            color: statusTab === tab ? '#249edc' : '#64748b',
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '24px', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ position: 'relative', width: '320px' }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', padding: '10px 12px 10px 36px', background: '#fff', border: '1px solid #cbd5e1', borderRadius: '8px', fontSize: '14px', color: '#0f172a', outlineColor: '#249edc', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}
                    />
                    {globalSearch && (
                                    ...
                                    <tr key={req.id} style={{ borderBottom: i === paginatedRequests.length - 1 ? 'none' : '1px solid #f1f5f9', background: '#fff', transition: 'background 0.15s' }} className="admin-tr">
                                        <td style={{ padding: '16px 24px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#e0f3fb', color: '#249edc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: '14px' }}>
