...
// Ensure snowflake_* tables exist (already created via migration script)
await pool.query(`
  ALTER TABLE public.snowflake_users ADD COLUMN IF NOT EXISTS is_superadmin bool DEFAULT false NULL;
`);
...
app.post('/api/reviews', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO snowflake_reviews (app_id, user_name, user_email, review_text)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
...
app.post('/api/users/sync', async (req, res) => {
  ...
  try {
    ...
    const existing = await pool.query(
      `SELECT user_id FROM public.snowflake_users WHERE LOWER(user_id) = $1 LIMIT 1`,
      [normalizedUserId]
    );
    ...
    if (existing.rows.length > 0) {
      ...
      if (rows.length === 0) {
        ({ rows } = await pool.query(
          `SELECT * FROM public.snowflake_users WHERE user_id = $1`,
          [storedUserId]
        ));
      }
    } else {
      ...
      ({ rows } = await pool.query(
        `INSERT INTO public.snowflake_users (user_id, name, email)
         VALUES ($1, $2, $3)
         RETURNING *`,
...
app.post('/api/users/activity', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO snowflake_user_activity_logs (user_id, name, email)
       VALUES ($1, $2, $3)
       RETURNING *`,
...
app.post('/api/users/activity/:id', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE snowflake_user_activity_logs
  ...
  try {
    ...
    let query = `
      SELECT id, user_id, name, email, opened_at, active_seconds
        FROM snowflake_user_activity_logs
...
app.post('/api/requests', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO snowflake_app_requests (user_name, user_email, app_id, app_name)
...
app.get('/api/requests', async (req, res) => {
  try {
    const { rows } = await pool.query(`SELECT * FROM snowflake_app_requests ORDER BY created_at DESC`);
    res.json(rows);
  } catch (err) {
    console.error(err);
...
app.put('/api/requests/:id', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE snowflake_app_requests SET status = $1 WHERE id = $2 RETURNING *`,
...
app.get('/api/access/:email', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT app_id, status FROM snowflake_app_requests WHERE user_email = $1`,
      [req.params.email]
    );
    res.json(rows);
  } catch (err) {
